...
"""
國家音樂廳 定價編輯器產生器
用法: python3 generate-editor-nch.py
輸出: editor-nch.html
...
SECTION_TYPES = {
    ...
    '2樓輪陪席': 'companion',
    '2樓友善席': 'accessible',
    '2樓友陪席': 'accessible_companion',
}

# 個別座位的特殊類型（以 section + row + seat 識別）
SEAT_TYPES = [
    # ── 工作席（8席，不販售）───────────────────────────────────────
    {'section': '2樓', 'row': '17', 'seat': 38, 'type': 'staff'},
    {'section': '2樓', 'row': '17', 'seat': 39, 'type': 'staff'},
    {'section': '2樓', 'row': '30', 'seat': 21, 'type': 'staff'},
    {'section': '2樓', 'row': '30', 'seat': 22, 'type': 'staff'},
    {'section': '3樓', 'row': '7',  'seat': 40, 'type': 'staff'},
    {'section': '3樓', 'row': '7',  'seat': 41, 'type': 'staff'},
    {'section': '4樓', 'row': '8',  'seat': 18, 'type': 'staff'},
    {'section': '4樓', 'row': '8',  'seat': 19, 'type': 'staff'},
    # ── 貴賓包廂（20席，需租借）────────────────────────────────────
    *[{'section': '3樓', 'row': 'VA', 'seat': s, 'type': 'vip_box'} for s in range(1, 11)],
    *[{'section': '3樓', 'row': 'VB', 'seat': s, 'type': 'vip_box'} for s in range(1, 11)],
    # ── 管風琴視線不良席（6席，2樓第29-31排外側）────────────────────
    {'section': '2樓', 'row': '29', 'seat': 38, 'type': 'organ_obstructed'},
    {'section': '2樓', 'row': '29', 'seat': 39, 'type': 'organ_obstructed'},
    {'section': '2樓', 'row': '30', 'seat': 37, 'type': 'organ_obstructed'},
    {'section': '2樓', 'row': '30', 'seat': 38, 'type': 'organ_obstructed'},
    {'section': '2樓', 'row': '31', 'seat': 30, 'type': 'organ_obstructed'},
    {'section': '2樓', 'row': '31', 'seat': 31, 'type': 'organ_obstructed'},
    # ── 座位不適且管風琴視線不良（12席，3樓10-11排外側）─────────────
    # 第10排（4席，座位25-28）
    *[{'section': '3樓', 'row': '10', 'seat': s, 'type': 'uncomfortable_obstructed'} for s in range(25, 29)],
    # 第11排（8席，座位25-32）
    *[{'section': '3樓', 'row': '11', 'seat': s, 'type': 'uncomfortable_obstructed'} for s in range(25, 33)],
    # ── 視線不良席────────────────────────────────────────────────
    # 2樓第27排（14席，座位24-37；22/23/38/39為輪椅陪同席）
    *[{'section': '2樓', 'row': '27', 'seat': s, 'type': 'obstructed'} for s in range(24, 38)],
    # 2樓第28排（18席，座位21-38）
    *[{'section': '2樓', 'row': '28', 'seat': s, 'type': 'obstructed'} for s in range(21, 39)],
    # 3樓第1排最外側（2席）
    {'section': '3樓', 'row': '1', 'seat': 9,  'type': 'obstructed'},
    {'section': '3樓', 'row': '1', 'seat': 10, 'type': 'obstructed'},
    # 4樓2E區全部（10席）
    *[{'section': '4樓', 'row': '2E', 'seat': s, 'type': 'obstructed'} for s in range(1, 11)],
    # 4樓第1排外側（4席）
    {'section': '4樓', 'row': '1', 'seat': 5, 'type': 'obstructed'},
    {'section': '4樓', 'row': '1', 'seat': 6, 'type': 'obstructed'},
    {'section': '4樓', 'row': '1', 'seat': 7, 'type': 'obstructed'},
    {'section': '4樓', 'row': '1', 'seat': 8, 'type': 'obstructed'},
    # 4樓第2排外側（4席）
    {'section': '4樓', 'row': '2', 'seat': 7,  'type': 'obstructed'},
    {'section': '4樓', 'row': '2', 'seat': 8,  'type': 'obstructed'},
    {'section': '4樓', 'row': '2', 'seat': 9,  'type': 'obstructed'},
    {'section': '4樓', 'row': '2', 'seat': 10, 'type': 'obstructed'},
]
...
const TYPE_COLOR = {
  ...
  organ_obstructed:        '#e07800',
  uncomfortable_obstructed:'#7f1d1d',
  obstructed:              '#b5895a',
};
const TYPE_LABEL = {
  regular:                 '一般席',
...
function displayColor(id) {
  const t = seatType[id];
  if (t !== 'regular' && t !== 'obstructed' && t !== 'organ_obstructed') return TYPE_COLOR[t];
...
function applyPrice() {
  ...
  selected.forEach(id => { if (seatType[id] !== 'staff') seatPrice[id] = price; });
...
function onHover(e) {
  ...
  const pStr = p ? 'NT$' + p.toLocaleString() : (t === 'staff' ? '不販售' : '未定價');
  tipEl.innerHTML = `<div class="tl">區域</div><div class="tv">${sec}</div>
...
function renderSpecialLegend() {
  const types = ['wheelchair','companion','accessible','accessible_companion',
                 'vip_box','organ_obstructed','uncomfortable_obstructed','obstructed','staff'];
...
function renderStats() {
  ...
  const staff   = Object.values(seatType).filter(t => t==='staff').length;
  const priced  = Object.keys(seatPrice).length;
  const revenue = Object.values(seatPrice).reduce((s,p) => s+p, 0);
  let html = `總席數：${total}<br>工作席：${staff}（不販售）<br>已定價：${priced}<br>未定價：${total-staff-priced}`;
...
function exportCSV() {
  ...
  a.download = 'NCH定價-' + new Date().toISOString().slice(0,10) + '.csv';
  a.click();
}
...
def main():
    src_path = '../../nch/nch.html'
    out_path = '../../nch/editor-nch.html'

    with open(src_path, encoding='utf-8') as f:
        content = f.read()

    # Extract canvas SVG (same logic as generate-nch.py)
