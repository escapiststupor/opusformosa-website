#!/usr/bin/env python3
"""
根據定價 CSV 更新 tct.html（觀眾瀏覽版）
用法: python3 apply-pricing.py <csv檔案>
範例: python3 apply-pricing.py TCT定價-2026-04-03.csv

CSV 格式（由 editor-tct.html 匯出）：
  section,row,seat,type,price
  2樓G21區,14,1,regular,2500
  2樓G21區輪椅席,10,11,wheelchair,1200
  ...
"""
...
def main(csv_path):
    ...
    # ── 讀 tct.html（已含 data 屬性的 SVG）────────────────────────────
    out_path = Path('../../tct/tct.html')
    ...
    def recolor(m):
        ...
        # 決定顏色
        NON_SALE = {'staff', 'recording', 'recording_view_affected', 'obstructed', 'pit'}
        if seat_type in TYPE_COLOR_HEX:
