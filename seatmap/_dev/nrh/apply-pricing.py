#!/usr/bin/env python3
"""
根據定價 CSV 更新 nrh.html（觀眾瀏覽版）
用法: python3 apply-pricing.py <csv檔案>
範例: python3 apply-pricing.py NRH定價-2026-04-03.csv

CSV 格式（由 editor-nrh.html 匯出）：
  section,row,seat,type,price
  地下樓BF,1,1,regular,2500
  地下樓BF輪椅席,10,11,wheelchair,1200
  ...
"""
...
def main(csv_path):
    ...
    # ── 讀 nrh.html（已含 data 屬性的 SVG）────────────────────────────
    out_path = Path('../../nrh/nrh.html')
    ...
    def recolor(m):
        ...
        if seat_type in NON_SALE:
            label = f'{TYPE_LABEL[seat_type]}（不販售）'
