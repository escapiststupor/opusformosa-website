...
範例: python3 update-excel.py NRH定價-2026-04-03.csv

需要安裝: pip install openpyxl

Excel 效果：
  原始的「國家兩廳院演奏廳空白圖」工作表不變。
  新增（或更新）一張「定價紀錄」工作表，格式如下：
    A: 區域  B: 排  C: 座位  D: 類型  E: 票價  F: 備註
  可用 VLOOKUP(A&B&C, 定價紀錄!A:F, 5, 0) 對照視覺圖。
"""
...
XLSX_PATH = Path('國家兩廳院演奏廳座位圖1150303(多元友善席專用).xlsx')
...
def main(csv_path):
    ...
    # ── 資料列 ──────────────────────────────────────────────────────
    NON_SALE = {'staff', 'photography'}
    for r, d in enumerate(rows_data, 2):
        type_zh = TYPE_LABEL_ZH.get(d['type'], d['type'])
        note    = ''
        if d['type'] in NON_SALE:     note = '不販售'
        elif 'obstructed' in d['type']: note = '視線受阻'
